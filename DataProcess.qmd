...
```
After filters, `urb_cpopcb` has `{r}nrow(ilc_dil6_filter)` rows, and `{r}ncol(ilc_dil6_filter)` columns.

We display xxx as below. Similarly, we only show some of the countries.
...
After filters, `urb_cpopcb` has `{r}nrow(ilc_di03_filter)` rows, and `{r}ncol(ilc_di03_filter)` columns.
...
```
After filters, `urb_cpopcb` has `{r}nrow(ilc_iw16_filter)` rows, and `{r}ncol(ilc_iw16_filter)` columns.

We display xxx as below.
the figure is the average income group by countries, country of birth and sex
