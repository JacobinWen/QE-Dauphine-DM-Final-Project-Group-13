...
Consider Different Countries will have different level of income, to make it comparable, we adopt a relative measurement, which is the ratio of `Income_FOR` to `Income_Dom`, defined as `Income_Rev`.
Secondly, in the educational aspect, given the sum of `ED0-2`, `ED3_4` and `ED5-8` is always equal to 1, to avoid the multicolinearity, we choose two of them as our independent variables.
Thirdly, for the sex variable, we take dummy variable, defined as 1 if `sex` is male, 0 if `sex` is female, denoted as `sex_onehot` 
Fourthly, for the `c_birth` variable, we also use dummy variable, defined as 1 if `c_birth` is `EU_FOR` and 0 is `c_birth` is `NEU_FOR`.
Fifthly, to ensure the data scale, we scale the Immigration_ratio by 100 times,define as `Imm_ratio`.
Finnaly, considering the data may have time effect (trend), we add lag variable of `Income_Rev` and `Imm_ratio`.
...
panelB_reg = panelB |> mutate(Income_Rev=Income_FOR/Income_Dom,
  ...
  mutate(Income_Rev_lag1=lag(Income_Rev, 1), Imm_ratio_lag1=lag(Imm_ratio,1)) |>
  mutate(Income_Rev_lag2=lag(Income_Rev, 2), Imm_ratio_lag2=lag(Imm_ratio,2)) |>
...
Hypothesis 1: the relative income level does not have gender effect.
Hypothesis 2: the relative income level does not have differences for birth of immigrants (EU or NonEU)
Hypothesis 3: the relative income level does not have education effect.
Hypothesis 4: the relative income level does not have clustering effect. (more immigrants, more competition, lower income)
...
summary(modelB)
```
From the regression table, we can see:
1. Relative Income level has strong autocorrelation
2. Relative Income level does not have significant gender effect, hence we could not reject hypothesis 1.
3. Country of birth has significantly positive correlation with relative income level, indicating that immigrants who are born in EU country has higher relative income than those who come from NonEU country. Therefore, we can reject hypothesis 2.
4. The fraction of immigrants within that group who receive low education is strongly negative correlated with relative income level. This shows that educational level will affect the income, which reject hypothesis 3.
5. Immigration Ratio have insignificant negative effect on the relative income, which cannot reject hypothesis 4.
